...
def main(args):
    ...
    for param in model.parameters():
        param.requires_grad = False         #역전파 파라미터 초기화.
    model.fc.weight.requires_grad = True   #가중치는 살린다.
    model.fc.bias.requires_grad = True      #편향값도 살린다.
    ...
    if args.resume:
        checkpoint = torch.load(args.resume, map_location="cuda", weights_only=True)# cpu -> cuda
    ...
    if args.test_only:
        ...
        if model_ema:
            epoch = 0
            evaluate(model_ema, criterion, data_loader_test, epoch ,device=device, log_suffix="EMA")
        else:
            epoch = 0
            evaluate(model, criterion, data_loader_test, epoch ,device=device)
        return
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.output_dir:
            ...
            if(epoch != 0):
                ...
                al = fig.add_subplot(2,1,1)
                al.plot(x_arr, to_numpy_train[0], '-o', label='Train loss')
                al.plot(x_arr, to_numpy_valid[0], '-->', label='Valid loss')
                handles, labels = plt.gca().get_legend_handles_labels()
                by_label = dict(zip(labels, handles))
                al.legend(by_label.values(), by_label.keys())
                #2행 1열의 데이터에서 2행으로 설정
                ar = fig.add_subplot(2,1,2)
                ar.plot(x_arr, to_numpy_train[1], '-o', label='Train acc')
                ar.plot(x_arr, to_numpy_valid[1], '-->', label='Valid acc')
                #중복 레이블 제거.
                handles, labels = plt.gca().get_legend_handles_labels()
                by_label = dict(zip(labels, handles))
                ar.legend(by_label.values(), by_label.keys())
                
                fig.xlabel('Epoch', size=15)
                fig.ylabel('Accuracy', size=15)
                
                fig.ion()
                #window.plot(fig)
                #fig.show(block=False) #멈춤 방지 계속해서 코드 진행.
                #fig.pause(1) #렉걸림 방지.
...
def get_args_parser(add_help=True):
    ...
    #crop-size가 영상 사이즈로 추정. 224*225로 바꾼다는 의미?
    parser.add_argument(
    ...
    parser.add_argument("--use-v2", action="store_true", help="Use V2 transforms") #증식 코드
    return parser
